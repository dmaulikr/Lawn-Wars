...
class GameScene: SKScene, SKPhysicsContactDelegate {
  ...
  var spawnThreshold: CFTimeInterval = 1.0
  ...
  func didBegin(_ contact: SKPhysicsContact) {
    ...
    if (contactA.categoryBitMask == 2 && contactB.categoryBitMask == 1) || (contactB.categoryBitMask == 2 && contactA.categoryBitMask == 1) {
      ...
      if contactA.categoryBitMask == 2 {
        ...
        marty.physicsBody?.velocity = CGVector(dx: 0, dy: 0)
        marty.physicsBody?.applyImpulse(CGVector(dx: 0, dy: impulseValue))
        nodeA.isHidden = true
        /* Increment points */
        points += 1
  ...
  func updateObstacles() {
    ...
    if spawnTimer >= spawnThreshold {
      ...
      self.spawnThreshold = 2.0 // <- Change me to be something random
      ...
      neighborBody.userData?.setValue(String(impulse), forKeyPath: "impulse")
